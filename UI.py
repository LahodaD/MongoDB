...
class App:
    ...
    def show_admin_layout(self):
        ...
        #logout_button.place(x=100,y=0)
        
        search_label = tk.Label(admin_frame, text="Search Books:")
        search_label.place(x=0,y=0)

        search_entry = tk.Entry(admin_frame)
        #search_entry.pack()
        search_entry.place(x=0,y=search_label.winfo_reqheight()+5)

        #search_button = tk.Button(admin_frame, text="Search", command=lambda: self.search_books(search_entry.get()))
        #search_button.pack()
        #search_button.place(x=search_entry.winfo_reqwidth()+5,y=search_entry.winfo_reqheight()+5)

        columns = ("Title", "Author", "Genre", "Copies")
        self.sort_order = {col: True for col in columns}  # Keep track of sorting order
        ...
        #self.admin_tree.pack(pady=55)
        self.admin_tree.place(x=0,y=search_label.winfo_reqheight() + search_entry.winfo_reqheight() + 15)
        ...
        # Add a key binding to delete selected rows when the Delete key is pressed
        ...
        height = self.admin_tree.winfo_reqheight() + search_label.winfo_reqheight() + search_entry.winfo_reqheight()
        width = self.admin_tree.winfo_reqwidth()
    ...
    def add_book(self, title, author, genre, copies, add_window):
        ...
        # Check if the book with the same title and author exists
        existing_item = None
        for item in self.admin_tree.get_children():
            if (
        ...
        if existing_item:
            ...
            self.admin_tree.item(existing_item, values=(title, author, genre, new_copies))
